-- =====================================================================
-- 011_slots_profissional.sql — get_available_slots por profissional
--
-- Substitui a versão de 007_slots_combo.sql, que calculava
-- disponibilidade para a barbearia inteira. Agora cada barbeiro
-- tem a própria agenda: expediente, folgas e agendamentos dele.
--
-- Bloqueio com profissional_id NULL = barbearia fechada (feriado),
-- então vale para todos os profissionais.
-- =====================================================================
...
-- Remove a versão antiga (sem profissional). A partir daqui escolher
-- o barbeiro é obrigatório — não existe mais "agenda da barbearia".
drop function if exists public.get_available_slots(text, date, text, boolean);
