-- =====================================================================
-- 012_slots_flex_end.sql — Atualiza get_available_slots para permitir
-- horários de agendamento até 30 minutos antes do almoço e fechamento
-- =====================================================================
...
begin
  ...
  -- 6. Itera a cada 30min até 30min antes do fechamento
  ...
  while v_current < v_fim_min loop
    ...
    loop
      if v_current < v_be and (v_current + v_step) > v_bs then
        v_blocked := true;
        exit;
      end if;
    end loop;
    ...
    loop
      if v_current < v_ae and (v_current + v_step) > v_as then
        v_booked := true;
        exit;
      end if;
    end loop;
