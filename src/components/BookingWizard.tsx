  ...
  return (
    <>
      ...
      {!isMobileOpen && (
        <div className="md:hidden text-center max-w-sm mx-auto px-4" id="agendar-sessao-teaser">
          <button
            ...
            className="w-full bg-[#c5a059] hover:bg-[#d6b472] text-black text-xs font-bold uppercase tracking-widest py-4 rounded-sm shadow-xl cursor-pointer flex items-center justify-center gap-2 transition-all hover:scale-[1.02] active:scale-[0.98]"
          >
      ...
      <div
        ...
        className={`text-slate-900 transition-all duration-300 md:relative md:inset-auto md:z-0 md:flex md:flex-col md:h-auto md:w-full bg-white border border-[#c5a059]/20 md:rounded-xl md:shadow-2xl md:overflow-hidden ${
          isMobileOpen
      ...
      >
        ...
        <div className="p-4 sm:p-6 bg-slate-50 border-b border-slate-200 flex items-center justify-between gap-4 shrink-0">
          <div className="text-left">
            ...
            <p className="text-slate-500 text-[10px] sm:text-xs mt-0.5 sm:mt-1 leading-tight text-left">Escolha seus serviços, marque o melhor dia e defina o horário</p>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            <div className="hidden xs:flex items-center gap-1.5 bg-[#c5a059]/10 text-[#c5a059] px-3 py-1.5 rounded-full border border-[#c5a059]/25 text-[9px] sm:text-[10px] font-mono uppercase tracking-wider shrink-0 font-bold">
        ...
        {step < 5 && (
          <div className="w-full bg-slate-100 h-1 shrink-0">
            <div 
              className="bg-gradient-to-r from-[#c5a059] to-[#b38e46] h-full transition-all duration-300" 
        ...
        >
        {errorMsg && (
          <div className="mb-4 p-3.5 bg-red-950/40 border border-red-900 text-red-400 rounded-sm text-xs flex items-center gap-2">
            <span className="font-bold uppercase text-[9px] tracking-wider bg-red-900/30 px-1 py-0.5 rounded-sm">Aviso:</span> {errorMsg}
          </div>
        )}
        ...
        <AnimatePresence mode="wait">
            ...
            >
              <div>
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 border-b border-slate-200 pb-3 mb-4">
                  <p className="text-[11px] text-slate-500 font-serif italic text-left">Selecione seu corte</p>
                </div>
                      ...
                      return (
                        <button
                          ...
                          className={`text-left p-5 rounded-md border transition-all duration-300 relative flex flex-col justify-between cursor-pointer group/card ${
                            isSelected 
                              ? 'bg-[#c5a059]/15 border-[#c5a059] shadow-md shadow-[#c5a059]/20' 
                              : 'bg-slate-50 hover:bg-slate-100 text-slate-800 border-slate-200'
                          }`}
                        >
                          <div className="w-full">
                            <div className="flex items-start justify-between gap-2">
                              <h4 className="font-serif font-semibold text-slate-900 text-sm flex items-center gap-2 group-hover/card:text-[#c5a059] transition-colors">
                                <Scissors className="w-3.5 h-3.5 text-[#c5a059] shrink-0" />
                                {s.nome}
                              </h4>
                              <span className="text-xs font-bold font-mono text-[#c5a059] text-gold-glow">
                                {getServicePrice(s) === 0 ? (
                                  <span className="text-[9px] uppercase bg-[#c5a059]/10 text-[#c5a059] border border-[#c5a059]/30 px-2 py-0.5 rounded-sm font-bold tracking-wider animate-pulse">Grátis (VIP)</span>
                                ) : (
                                  formatBRL(s.preco)
                                )}
                              </span>
                          ...
                          <div className="mt-3 pt-3 border-t border-slate-200 flex items-center justify-between text-[10px] uppercase font-mono tracking-wider text-slate-500 w-full">
                            ...
                            <div className="flex items-center gap-1.5">
                              {isSelected ? (
                                <span className="bg-[#c5a059] text-black rounded-md px-2 py-0.5 text-[9px] font-black flex items-center gap-0.5">
                                  ✓ Selecionado
                                </span>
                              ) : (
                                <span className="text-slate-500 group-hover/card:text-slate-700 transition text-[9px] font-bold">
                                  + Adicionar
                                </span>
                              )}
                            </div>
              ...
              <div className="hidden md:block space-y-4 mt-6">
                {selectedServices.length > 0 && (
                  <div className="p-3.5 bg-slate-50 border border-slate-200 rounded-md flex items-center justify-between text-xs font-mono text-slate-700">
                    <span className="text-slate-500 text-[10px] uppercase">Pacote Selecionado:</span>
                    <div className="flex items-center gap-3">
                      <span className="text-slate-500">{totalDuracao} min</span>
                      <span className="text-[#c5a059] font-bold font-sans text-sm">{formatBRL(totalPreco)}</span>
                    </div>
                  </div>
                )}
                ...
                <div className="pt-4 border-t border-slate-200 flex justify-end">
                  <button
                    ...
                    className="bg-gradient-to-r from-[#c5a059] to-[#b38e46] hover:from-[#d6b472] hover:to-[#c5a059] text-black disabled:opacity-50 text-[10px] uppercase tracking-widest font-black px-6 py-3.5 rounded-md flex items-center justify-center gap-2 cursor-pointer shadow-lg shadow-[#c5a059]/10 transition-all duration-300 hover:scale-105 active:scale-95"
                  >
          ...
          {step === 2 && (
            ...
            >
              <div>
                <p className="text-[11px] text-slate-500 mb-3 font-serif italic text-left">Escolha seu dia</p>
                ...
                <div className="bg-slate-50/80 border-y border-slate-200 -mx-3 xs:-mx-5 sm:-mx-6 px-3 xs:px-5 sm:px-6 py-4 md:py-6 space-y-4 shadow-inner w-auto">
                  ...
                  <div className="flex items-center justify-between border-b border-slate-200 pb-3">
                    <button
                      ...
                      className="p-2 rounded-md hover:bg-stone-900 border border-slate-200 hover:border-slate-200 hover:text-[#c5a059] transition-all text-slate-500 cursor-pointer text-xs font-bold font-mono"
                      title="Mês Anterior"
                    >
                      &larr;
                    </button>
                    <span className="font-serif text-sm text-[#c5a059] font-semibold tracking-wide text-gold-glow">
                  ...
                  <div className="grid grid-cols-7 gap-1 text-center font-sans text-[9px] font-bold tracking-widest uppercase text-slate-500">
                      ...
                      return (
                        <button
                          ...
                          className={`w-full py-3 md:py-5 text-xs font-sans rounded-md transition-all flex flex-col items-center justify-center relative cursor-pointer group/day ${
                            isSelected
                              ? 'bg-[#c5a059]/15 border border-[#c5a059] shadow-md shadow-[#c5a059]/20 scale-105 z-10'
                              : isPast
                        ...
                        >
                          <span>{cell}</span>
                          {isToday && !isSelected && (
                            <span className="absolute bottom-1.5 w-1.5 h-1.5 bg-[#c5a059] rounded-full animate-ping" />
                          )}
                        </button>
                      );
                    })}
              ...
              <div className="hidden md:block space-y-4 mt-6">
                {selectedServices.length > 0 && (
                  <div className="bg-slate-50 p-3.5 rounded-md border border-slate-200 text-xs flex flex-wrap items-center justify-between gap-1 text-slate-700 leading-normal">
                    <span className="text-slate-500 font-mono text-[10px] uppercase">Serviços Escolhidos:</span>
                ...
                <div className="pt-4 border-t border-slate-200 flex flex-row items-center justify-between gap-3 w-full">
                  <button
                    ...
                    className="flex-1 md:flex-none border border-slate-200 text-slate-500 hover:bg-stone-900 hover:text-slate-900 text-[10px] uppercase tracking-wider font-bold py-3.5 md:px-6 rounded-md flex items-center justify-center gap-1.5 transition cursor-pointer font-mono"
                  >
                  ...
                  <button
                    ...
                    className="flex-1 md:flex-none bg-gradient-to-r from-[#c5a059] to-[#b38e46] hover:from-[#d6b472] hover:to-[#c5a059] text-black disabled:opacity-50 text-[10px] uppercase tracking-widest font-black py-3.5 md:px-6 rounded-md flex items-center justify-center gap-2 cursor-pointer shadow-lg shadow-[#c5a059]/10 transition-all duration-300 hover:scale-105 active:scale-95"
                  >
                    Continuar <ArrowRight className="w-3.5 h-3.5" />
                  </button>
                </div>
          ...
          {step === 3 && (
            ...
            >
              <div>
                <p className="text-[11px] text-slate-500 mb-3 font-serif italic text-left">Selecione o seu horário</p>
                ...
                {loadingSlots ? (
                  <div className="py-12 text-center space-y-3 bg-slate-50 rounded-sm border border-slate-200">
                    ...
                    <p className="text-[10px] text-slate-500 font-mono uppercase tracking-wider">Buscando horários livres com o barbeiro...</p>
                  </div>
                ) : availableSlots.length === 0 ? (
                  <div className="py-8 bg-slate-100 border border-stone-850/80 p-5 rounded-sm text-center">
                    <p className="text-xs text-[#c5a059] font-serif leading-relaxed">
                    ...
                    <p className="text-[10px] text-slate-500 font-mono uppercase tracking-wider mt-3">Dica: Tente escolher outro dia ou fale conosco.</p>
                  </div>
                ) : (
                        ...
                        return (
                          <button
                            ...
                            className={`py-3 px-2.5 rounded-md border text-xs font-sans font-bold transition-all duration-200 relative flex flex-col items-center justify-center cursor-pointer ${
                              isSelected
                                ? 'bg-[#c5a059]/15 border-[#c5a059] shadow-md shadow-[#c5a059]/20 scale-[1.04] z-10'
                                : isFree
                                  ? 'bg-slate-50 hover:bg-slate-100 text-slate-800 border-slate-200'
                                  : slotObj.motivo === 'ocupado'
                                    ? 'bg-red-950/5 text-stone-650 border-red-950/15 line-through cursor-not-allowed opacity-40'
                          ...
                          >
                            ...
                            {!isFree && (
                              <span className="text-[8px] tracking-tight uppercase font-sans mt-0.5 opacity-80 block truncate text-stone-555 font-mono">
              ...
              <div className="hidden md:block space-y-4 mt-6">
                {selectedServices.length > 0 && selectedDate && (
                  <div className="bg-slate-50 p-3.5 rounded-sm border border-slate-200 text-[11px] space-y-1.5">
                    <div className="flex justify-between md:items-center flex-col md:flex-row gap-1">
                      <span className="text-slate-500 font-mono text-[10px] uppercase">Serviços Selecionados:</span>
                      <span className="font-semibold text-slate-900">{selectedServices.map(s => s.nome).join(' + ')}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-500 font-mono text-[10px] uppercase">Data selecionada:</span>
                      <span className="font-semibold text-slate-900 capitalize">{getWeekDayName(selectedDate)} ({selectedDate.split('-').reverse().join('/')})</span>
                    </div>
                  </div>
                )}

                <div className="pt-4 border-t border-stone-850/80 flex flex-row items-center justify-between gap-3 w-full">
                  <button
                    type="button"
                    onClick={handleBackStep}
                    className="flex-1 md:flex-none border border-stone-800 text-slate-500 hover:bg-stone-900 hover:text-slate-900 text-[10px] uppercase tracking-wider font-bold py-3 md:px-5 rounded-sm flex items-center justify-center gap-1.5 transition cursor-pointer font-mono"
                  >
                    <ArrowLeft className="w-3.5 h-3.5" /> Voltar
                  </button>
                  <button
                    ...
                    className="flex-1 md:flex-none bg-[#c5a059] hover:bg-[#d6b472] text-black disabled:opacity-50 text-[10px] uppercase tracking-widest font-bold py-3 md:px-5 rounded-sm flex items-center justify-center gap-1.5 cursor-pointer shadow-md"
                  >
                    Continuar <ArrowRight className="w-3.5 h-3.5" />
                  </button>
                </div>
          ...
          {step === 4 && (
            ...
            >
              <form id="booking-wizard-form" onSubmit={handleBookNow} className="space-y-4">
                <div>
                  <p className="text-[11px] text-slate-500 mb-3 font-serif italic text-left">Preencha seus dados</p>
                  ...
                  <div className="space-y-3.5">
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full pl-10 pr-4 py-3.5 bg-white border border-slate-200 rounded-md text-xs focus:outline-none focus:ring-1 focus:ring-[#c5a059]/20 focus:border-[#c5a059] text-slate-900 placeholder:text-slate-400 transition"
                      />
                    </div>
                    ...
                    <div>
                      <textarea
                        ...
                        className="w-full p-3.5 bg-white border border-slate-200 rounded-md text-xs focus:outline-none focus:ring-1 focus:ring-[#c5a059]/20 focus:border-[#c5a059] text-slate-900 placeholder:text-slate-400 transition"
                      />
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-slate-50 p-5 rounded-md border border-slate-200 text-xs space-y-3.5 shadow-md">
                  <h5 className="font-mono font-bold text-[#c5a059] text-[10px] tracking-widest uppercase">Resumo da Reserva:</h5>
                  <div className="grid grid-cols-2 gap-y-2.5 text-slate-500">
                    ...
                    <div>Horário:</div>
                    <div className="font-semibold text-right font-mono text-[#c5a059] text-sm">{selectedSlot}h</div>

                    <div className="pt-2.5 border-t border-slate-200 font-bold text-[#c5a059] uppercase tracking-wider text-[9px] font-mono">Preço Total:</div>
                    <div className="pt-2.5 border-t border-slate-200 text-right font-serif font-bold text-[#c5a059] text-lg text-gold-glow">
                ...
                <div className="hidden md:block pt-4 border-t border-slate-200">
                  <div className="flex flex-row items-center justify-between gap-3 w-full">
                    ...
                    <button
                      type="submit"
                      disabled={submitting}
                      className="flex-1 md:flex-none bg-gradient-to-r from-[#c5a059] to-[#b38e46] hover:from-[#d6b472] hover:to-[#c5a059] text-black disabled:opacity-50 text-[10px] uppercase tracking-widest font-black py-3.5 md:px-6 rounded-md flex items-center justify-center gap-2 cursor-pointer shadow-lg shadow-[#c5a059]/10 transition-all duration-300 hover:scale-105 active:scale-95 font-mono"
                    >
                      {submitting ? (
                        <>Reservando...</>
                      ) : (
          ...
          {step === 5 && successBooking && (
            ...
            >
              <div className="w-12 h-12 bg-emerald-950/30 rounded-md flex items-center justify-center text-emerald-400 mx-auto border border-emerald-900/50 shadow-lg animate-bounce">
              ...
              <div className="bg-slate-50 border border-slate-200 p-5 rounded-lg max-w-sm mx-auto text-xs space-y-2.5 text-left shadow-md">
                <div className="text-[10px] uppercase tracking-widest text-[#c5a059] font-mono font-bold mb-1.5">Detalhes do Voucher:</div>
                <div className="flex justify-between border-b border-slate-200/40 pb-1.5"><span className="text-slate-500 font-mono">Código:</span> <span className="font-mono font-bold text-[#c5a059]">{successBooking.id}</span></div>
                ...
                <div className="flex justify-between border-b border-slate-200/40 pb-1.5"><span className="text-slate-500">Quando:</span> <span className="font-bold text-[#c5a059]">{selectedDate.split('-').reverse().join('/')} às {selectedSlot}h</span></div>
                ...
                <div className="flex justify-between border-t border-slate-200 pt-2 font-bold"><span className="text-slate-500 font-mono text-[10px]">Valor fixado total:</span> <span className="font-serif font-bold text-[#c5a059] text-base text-gold-glow">{formatBRL(totalPreco)}</span></div>
              </div>
              ...
              <div className="pt-4 flex flex-col sm:flex-row items-center justify-center gap-3">
                <button
                  ...
                  className="w-full sm:w-auto bg-[#090807] hover:bg-[#0c0a09] text-[#c5a059] border border-slate-200 hover:border-slate-200 text-[10px] uppercase tracking-wider font-bold px-6 py-3.5 rounded-md transition duration-200 cursor-pointer flex items-center justify-center gap-2 font-mono shadow-md"
                >
                  <Printer className="w-4 h-4 text-[#c5a059]" /> Imprimir PDF
                </button>
                <button
                  type="button"
                  onClick={resetWizard}
                  className="w-full sm:w-auto bg-gradient-to-r from-[#c5a059] to-[#b38e46] hover:from-[#d6b472] hover:to-[#c5a059] text-black text-[10px] uppercase tracking-widest font-black px-6 py-3.5 rounded-md transition-all duration-300 cursor-pointer flex items-center justify-center gap-1.5 font-mono shadow-lg shadow-[#c5a059]/10 hover:scale-105 active:scale-95"
                >
          ...
          <div className="md:hidden shrink-0 bg-slate-100/90 border-t border-slate-200/80 p-4 xs:p-5 sm:px-6 sm:py-5 space-y-3.5 z-20 backdrop-blur-md">
            ...
            {selectedServices.length > 0 && step < 4 && (
              <div className="p-4 bg-slate-50 border border-slate-200 text-slate-800 rounded-md">
                <span className="text-slate-500 font-mono text-[9px] uppercase tracking-wider block text-left">
                  SERVIÇOS ESCOLHIDOS:
                </span>
                <p className="font-serif text-[#c5a059] text-xs sm:text-sm font-semibold tracking-wide text-left mt-1.5 leading-relaxed text-gold-glow">
            ...
            <div className="flex items-center justify-between gap-4">
              {step > 1 && (
                <button
                  ...
                  className="flex-1 border border-slate-200 text-slate-500 hover:bg-stone-900 hover:text-white text-[10px] uppercase tracking-widest font-bold py-3.5 px-5 rounded-md flex items-center justify-center gap-1.5 transition cursor-pointer font-mono"
                >
                  <ArrowLeft className="w-3.5 h-3.5" /> Voltar
                </button>
              )}
              <button
                ...
                className="flex-1 bg-gradient-to-r from-[#c5a059] to-[#b38e46] hover:from-[#d6b472] hover:to-[#c5a059] text-black disabled:opacity-40 disabled:cursor-not-allowed text-[10px] uppercase tracking-widest font-black py-3.5 px-5 rounded-md flex items-center justify-center gap-1.5 cursor-pointer shadow-lg shadow-[#c5a059]/10 transition duration-350 font-mono active:scale-95"
              >
                {step === 4 ? (
      ...
      {showGoogleLoginPopup && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className="relative bg-[#161616] border border-stone-850 rounded-sm w-full max-w-sm p-6 text-center space-y-5 shadow-2xl overflow-hidden"
          >
            <div className="w-12 h-12 rounded-full bg-stone-900 border border-stone-800 flex items-center justify-center mx-auto text-slate-700">
            ...
            <div className="space-y-2.5 pt-1">
              <button
                ...
                className="w-full py-3 bg-slate-100 hover:bg-stone-900 border border-slate-200 text-slate-900 text-xs font-bold uppercase tracking-widest rounded-sm transition cursor-pointer flex items-center justify-center font-mono gap-2 relative shadow-md"
              >
                {isGoogleLoading ? (
                  <>
                    <div className="w-4 h-4 border-2 border-[#c5a059] border-t-transparent rounded-full animate-spin shrink-0"></div>
                    Autenticando...
                  </>
                ) : (
                  <>
              ...
              <button
                ...
                disabled={isGoogleLoading}
                onClick={() => setShowGoogleLoginPopup(false)}
                className="w-full py-2.5 text-[10px] uppercase font-bold tracking-wider text-stone-550 hover:text-stone-350 transition cursor-pointer font-mono"
              >
                Continuar sem login
              </button>
            </div>
